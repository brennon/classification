...
def enhancedFeatureExtractorDigit(datum):
    ...
    # Get ratio of white pixels to black pixels.
    whitePixels = []
    blackPixels = []
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
    ...
    # # Feature for more than half white pixels.
    # ratio = float(len(whitePixels)) / (len(whitePixels) + len(blackPixels))
    # if ratio > 0.5:
    #     features["whiteRatio"] = 1
    # else:
    #     features["moreThanHalfWhite"] = 0

    # # Create 4x4 superpixels.
    # superpixels = util.Counter()
    # for x in range(DIGIT_DATUM_WIDTH):
    #     xMod = x / 4
    ...
    #         indexString = "superpixel ({0}, {1})".format(xMod, yMod)

    #         if datum.getPixel(x, y) > 0:
    #             superpixels[indexString] += 1
    #         else:
    #             superpixels[indexString] = superpixels[indexString]

    # requiredForHalfBlack = 8
    # for x in range(DIGIT_DATUM_WIDTH / 4):
    #     for y in range(DIGIT_DATUM_HEIGHT / 4):
    #         indexString = "superpixel ({0}, {1})".format(x, y)
    #         if superpixels[indexString] >= 8:
    #             superpixels[indexString] = 1
    #         else:
    #             superpixels[indexString] = 0
    #         features[indexString] = superpixels[indexString]

    # Count white areas.
    frontier = util.Queue()
    visited = set()
    whiteAreas = 0
    whitePixelsForSearch = list(whitePixels)
    while len(whitePixelsForSearch) != 0:
        nextWhitePixel = whitePixelsForSearch.pop()
        if nextWhitePixel not in visited:
            visited.add(nextWhitePixel)
            frontier.push(nextWhitePixel)
            whiteAreas += 1
        while not frontier.isEmpty():
            x, y = frontier.pop()

            candidateNeighbors = [(x-1,y), (x+1,y), (x,y+1), (x,y-1)]
            for candidateNeighbor in candidateNeighbors:
                if candidateNeighbor in whitePixelsForSearch:
                    if candidateNeighbor not in visited:
                        visited.add(candidateNeighbor)
                        frontier.push(candidateNeighbor)

    for i in range(1,7):
        indexString = "whiteSpaces-{0}".format(i)
        if i == whiteAreas:
            features[indexString] = 1
        else:
            features[indexString] = 0

    # # Find vertical symmetry.
    # blackPixelsRemaining = []
    # for x in range(DIGIT_DATUM_WIDTH / 2):
    #     for y in range(DIGIT_DATUM_HEIGHT):
    #         verticalOppositeX, verticalOppositeY = getVerticalSymmetricCoordinate((x, y), DIGIT_DATUM_WIDTH, DIGIT_DATUM_HEIGHT)

    #         thisPixel = datum.getPixel(x, y)
    #         oppositePixel = datum.getPixel(verticalOppositeX, verticalOppositeY)

    #         if thisPixel > 0:
    #             if oppositePixel == 0:
    #                 blackPixelsRemaining.append(thisPixel)
    #                 blackPixelsRemaining.append(oppositePixel)
    #         elif thisPixel == 0:
    #             if oppositePixel > 0:
    #                 blackPixelsRemaining.append(thisPixel)
    #                 blackPixelsRemaining.append(oppositePixel)
    # verticalSymmetryRatio = 1 - (float(len(blackPixelsRemaining)) / (DIGIT_DATUM_HEIGHT * DIGIT_DATUM_WIDTH))

    # for bucket in range(1,8):
    #     bucketBottom = 60 + bucket * 5
    #     bucketTop = bucketBottom + 5
    #     indexString = "verticalSymmetry{0}-{1}".format(bucketBottom, bucketTop)
    #     if verticalSymmetryRatio * 100 > bucketBottom and verticalSymmetryRatio * 100 <= bucketTop:
    #         features[indexString] = 1
    #     else:
    #         features[indexString] = 0
    ...
    #         horizontalOppositeX, horizontalOppositeY = getHorizontalSymmetricCoordinate((x, y), DIGIT_DATUM_WIDTH, DIGIT_DATUM_HEIGHT)

    #         thisPixel = datum.getPixel(x, y)
    #         oppositePixel = datum.getPixel(horizontalOppositeX, horizontalOppositeY)
    ...
    #                 blackPixelsRemaining.append(thisPixel)
    #                 blackPixelsRemaining.append(oppositePixel)
    # horizontalSymmetryRatio = 1 - (float(len(blackPixelsRemaining)) / (DIGIT_DATUM_HEIGHT * DIGIT_DATUM_WIDTH))

    # for bucket in range(1,8):
    #     bucketBottom = 60 + bucket * 5
    #     bucketTop = bucketBottom + 5
    #     indexString = "horizontalSymmetry{0}-{1}".format(bucketBottom, bucketTop)
    #     if horizontalSymmetryRatio * 100 > bucketBottom and horizontalSymmetryRatio * 100 <= bucketTop:
    #         features[indexString] = 1
    #     else:
    #         features[indexString] = 0
...
def getVerticalSymmetricCoordinate(point, width, height):
    originalX, originalY = point
    return (width - originalX - 1, originalY)

def getHorizontalSymmetricCoordinate(point, width, height): 
    originalX, originalY = point
    return (originalX, height - originalY - 1)
...
def analysis(classifier, guesses, testLabels, testData, rawTestData, printImage):
    ...
    #     print(rawTestData[i])
    #     print("")
    #     print("Predicted label: {0}, Actual label: {1}".format(guesses[i], testLabels[i]))
    #     print("White areas: {0}".format(testData[i]['whiteAreas']))
    #     print("White ratio: {0}".format(testData[i]['whiteRatio']))
    #     for bucket in range(1,8):
    #         bucketBottom = 60 + bucket * 5
    #         bucketTop = bucketBottom + 5
    #         indexString = "verticalSymmetry{0}-{1}".format(bucketBottom, bucketTop)
    #         print("{0}: {1}".format(indexString, testData[i][indexString]))
    #     for bucket in range(1,8):
    #         bucketBottom = 60 + bucket * 5
    #         bucketTop = bucketBottom + 5
    #         indexString = "horizontalSymmetry{0}-{1}".format(bucketBottom, bucketTop)
